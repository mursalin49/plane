...
class _TroubleScreenState extends State<TroubleScreen>
  ...
  Widget _buildContinueButton() {
    return GestureDetector(
      onTap: () {
        final selected = controller.selectedIssue.value;
        if (selected == 'email_issue') {
          Get.to(() => const ContactManagementScreen());
        } else if (selected == 'id_issue' || selected == 'pass_issue') {
          Get.to(() => ForgotPasswordScreen());
        } else {
          Get.snackbar('', 'Please select an issue first',
...
class _ContactManagementScreenState extends State<ContactManagementScreen>
  ...
  void initState() {
    ...
    _c2Scale = TweenSequence<double>([
      TweenSequenceItem(tween: Tween(begin: 0.35, end: 1.0).chain(CurveTween(curve: Curves.easeOutCubic)), weight: 40),
      TweenSequenceItem(tween: ConstantTween(1.0), weight: 15),
      TweenSequenceItem(tween: Tween(begin: 1.0, end: 0.35).chain(CurveTween(curve: Curves.easeInCubic)), weight: 20),
      TweenSequenceItem(tween: ConstantTween(0.35), weight: 25),
    ]).animate(_waveCtrl);
    ...
    _c3Scale = TweenSequence<double>([
      TweenSequenceItem(tween: ConstantTween(0.35), weight: 42),
      TweenSequenceItem(tween: Tween(begin: 0.35, end: 1.0).chain(CurveTween(curve: Curves.easeOutCubic)), weight: 28),
      TweenSequenceItem(tween: ConstantTween(1.0), weight: 10),
      TweenSequenceItem(tween: Tween(begin: 1.0, end: 0.35).chain(CurveTween(curve: Curves.easeInCubic)), weight: 20),
    ]).animate(_waveCtrl);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Container(
            ...
            child: Stack(
              ...
              children: [
                // Circle 1 — fixed
                Positioned(
                  top: -(c1Size * 0.38), right: -(c1Size * 0.30),
                  child: Container(
                    width: c1Size, height: c1Size,
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                ...
                // Circle 3 — wave big
                Positioned(
                  top: -(c3Size * 0.38), right: -(c3Size * 0.30),
                  child: AnimatedBuilder(
                    animation: _waveCtrl,
                    builder: (_, __) => Opacity(
                      opacity: _c3Opacity.value,
                      child: Transform.scale(
                        scale: _c3Scale.value, alignment: Alignment.topRight,
                        child: Container(
                          width: c3Size, height: c3Size,
                          decoration: BoxDecoration(
                            shape: BoxShape.circle,
                ...
                Positioned(
                  top: -(c2Size * 0.38), right: -(c2Size * 0.30),
                  child: AnimatedBuilder(
                    animation: _waveCtrl,
                    builder: (_, __) => Opacity(
                      opacity: _c2Opacity.value,
                      child: Transform.scale(
                        scale: _c2Scale.value, alignment: Alignment.topRight,
                        child: Container(
                          width: c2Size, height: c2Size,
                          decoration: BoxDecoration(
                            shape: BoxShape.circle,
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          SizedBox(width: 8.w),
                          Text('Parallax',
                              style: GoogleFonts.dmSans(fontSize: 15.sp, fontWeight: FontWeight.w400, color: Colors.white, letterSpacing: -0.2)),
                        ],
                      ),
                      SizedBox(height: 24.h),
                      Text(
                        ...
                        style: GoogleFonts.dmSans(
                          ...
                          color: Colors.white, letterSpacing: -0.6, height: 1.2,
                        ),
                      ),
                      SizedBox(height: 32.h),
                    ],
          ...
          // White Card — compact, not expanded
          Transform.translate(
            offset: const Offset(0, -30),
            child: Container(
              margin: EdgeInsets.symmetric(horizontal: 16.w),
  ...
  Widget _buildHeroMark() {
    return SizedBox(
      ...
      child: Row(
        ...
        crossAxisAlignment: CrossAxisAlignment.end,
        children: [
          _bar(18.h, 1.0), SizedBox(width: 3.w),
          _bar(13.h, 0.4), SizedBox(width: 3.w),
          _bar(9.h, 0.15),
        ],
      ),
    );
