...
class OtpIdVerificationScreen extends StatefulWidget {
  const OtpIdVerificationScreen({required this.email, super.key});

  final String email;

  @override
  State<OtpIdVerificationScreen> createState() =>
      _OtpIdVerificationScreenState();
}

class _OtpIdVerificationScreenState extends State<OtpIdVerificationScreen> {
  ...
  Future<void> _handleContinue() async {
    ...
    try {
      final userId = await _authController.verifyUidRecoveryCode(code);
      if (!mounted) {
        return;
      }
      Get.off(() => OtpIdSuccessScreen(recoveredUserId: userId));
  ...
  Future<void> _handleResend() async {
    try {
      await _authController.resendUidRecoveryCode();
      _startResendTimer();
